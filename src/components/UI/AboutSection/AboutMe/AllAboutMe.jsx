...
const AllAboutMes = () => {
  ...
  const { data, error, isLoading, refetch } = useGetAllAboutMeQuery(query);
  const [deleteAboutMe] = useDeleteAboutMeMutation();

  // Filters aboutMe based on the search query
  const filteredData = useMemo(() => data?.data?.data || [], [data]);

  const handleDeleteAboutMe = async (aboutMe) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the aboutMe "${aboutMe?.title}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteAboutMe(aboutMe?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The aboutMe "${aboutMe?.title}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
      ...
      <div className="bg-[#0D0E12] sticky top-[75px] md:top-[82px] z-[450] py-2 md:py-0">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 ">
          ...
          <div className="flex items-center w-full justify-between md:justify-end gap-4">
            ...
            <Link href="/about-page/about-me/add-about-me">
              <button className="btn w-[150px] md:w-64">Add AboutMe</button>
            </Link>
          </div>
        </div>
      </div>
      <div className="mx-auto w-full pt-2 mt-6">
        <h1 className="table_header">All AboutMes</h1>
            ...
            <table className="w-full">
              <thead>
                <tr className="table_row ">
                  ...
                  <th className="table_th">sub_title</th>
                  <th className="table_th">description</th>
                  <th className="table_th">cv_link</th>
                ...
                {filteredData?.length > 0 ? (
                  filteredData?.map((item, index) => (
                    <tr key={index} className="tbody_tr">
                      ...
                      <td className="table_th">
                        <p>{truncateCharacters(item?.sub_title, 30)}</p>
                      </td>
                      <td className="table_th">
                        <p>{truncateCharacters(item?.description, 30)}</p>
                      </td>
                      <td className="table_th">
                        <p>{truncateCharacters(item?.cv_link, 30)}</p>
                      </td>

                      <td className="my-2 px-4 text-center ">
                        <div className="flex items-center justify-center w-full gap-2">
                          <EditIcon
                            edit_link={`/about-page/about-me/edit-about-me/${item?.id}`}
                          />
                          <DeleteIcon
