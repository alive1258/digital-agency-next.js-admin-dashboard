...
const AllProjectCategories = () => {
  ...
  const { data, error, isLoading, refetch } =
    useGetAllProjectCategoriesQuery(searchValue);
  ...
  const handleDeleteProjectCategory = async (faq) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the All ProjectCategory "${faq?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteProjectCategory(faq?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The AllProjectCategory "${faq?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
    ...
    <section className="md:px-6 px-4 mt-6 rounded-lg">
      ...
      <div className="flex items-center justify-between space-x-4">
        ...
        <div className="flex items-center space-x-4">
          ...
          <Link href="/projects/project-categories/add-project-category">
            <button className="btn w-64">Add Project Category</button>
          </Link>
        </div>
      </div>
      <div className="mx-auto w-full pt-4">
        <div className="overflow-x-auto w-full">
          <h1 className="table_header">All Project Category</h1>
          <div className="table_section">
            <table className="w-full">
              <thead>
                <tr className="table_row">
                  <th className="table_th w-4">#SL</th>
                  <th className="table_th ">Project Category Name</th>
                    ...
                    <tr key={item?.id} className="tbody_tr">
                      ...
                      <td className="my-2 px-4 text-center ">
                        <div className="flex items-center justify-center w-full gap-4">
                          <EditIcon
                            edit_link={`/projects/project-categories/edit-project-category/${item?.id}`}
                          />
                          <DeleteIcon
                            handleDelete={handleDeleteProjectCategory}
                            item={item}
                          />
                        </div>
                      </td>
