...
import { useDeleteFaqMutation, useGetAllFaqsQuery } from "@/redux/api/faqsApi";
...
const AllFaqs = () => {
  ...
  const { data, error, isLoading, refetch } = useGetAllFaqsQuery(searchValue);
  const [deleteInvestor] = useDeleteFaqMutation();
  ...
  const handleDeleteFaq = async (faq) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the faq "${faq?.headline}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteInvestor(faq?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The faq "${faq?.headline}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
                  ...
                  filteredData?.map((item, index) => (
                    <tr key={item?.id} className="tbody_tr">
                      ...
                      <td className="table_th">
                        <p>{truncateCharacters(item?.headline, 30)}</p>
                      </td>

                      <td className="my-2 px-4 text-center ">
                        <div className="flex items-center justify-center w-full gap-4">
                          <EditIcon edit_link={`/faqs/edit-faq/${item?.id}`} />
                          <DeleteIcon
